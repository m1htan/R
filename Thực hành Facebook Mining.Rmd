...
# Chuyển ma trận từ - tài liệu thành dạng dữ liệu phù hợp
tdm_df <- as.data.frame(as.matrix(myTdm))
tdm_df$term <- rownames(tdm_df)
tdm_df_long <- melt(tdm_df, id.vars = "term", variable.name = "Document", value.name = "Frequency")

# Tạo biểu đồ
ggplot(tdm_df_long, aes(x = term, y = Document, fill = Frequency)) +
