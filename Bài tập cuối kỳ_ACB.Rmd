...
```{r}
ACB_1 <- subset(ACB, select = c(date, time, open, high, low, close, nmVolume))
head(ACB_1)
```
...
cor(ACB_1$open, ACB_1$high, use = "complete.obs")
ggplot(ACB_1, aes(x=open, y=high)) + geom_point()

# Tính hệ số tương quan Pearson giữa hai cột open và low
cor(ACB_1$open, ACB_1$low, use = "complete.obs")
ggplot(ACB_1, aes(x=open, y=low)) + geom_point()

# Tính hệ số tương quan Pearson giữa hai cột high và close
cor(ACB_1$high, ACB_1$close, use = "complete.obs")
ggplot(ACB_1, aes(x=high, y=close)) + geom_point()

# Tính hệ số tương quan Pearson giữa hai cột low và close
cor(ACB_1$low, ACB_1$close, use = "complete.obs")
ggplot(ACB_1, aes(x=low, y=close)) + geom_point()
```
...
boxplot(ACB_3$open, main="Boxplot of 'open' Column", ylab="open Price")
boxplot(ACB_3$high, main="Boxplot of 'high' Column", ylab="high Price")
boxplot(ACB_3$low, main="Boxplot of 'low' Column", ylab="low Price")
boxplot(ACB_3$close, main="Boxplot of 'close' Column", ylab="close Price")
