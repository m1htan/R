...
```{r}
VCB_1 <- subset(ACB, select = c(date, time, open, high, low, close, nmVolume))
head(VCB_1)
```
...
VCB_1[VCB_1$date %in% n_occur$Var1[n_occur$Freq > 1],]
```

```{r}
# Xoá dữ liệu trùng lặp
VCB_2 <- VCB_1[!duplicated(ACB_1$date), ]
VCB_2
```
...
boxplot(ACB_2$open, ACB_3$close, ACB_3$high, ACB_3$low,
